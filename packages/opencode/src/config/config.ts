...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/elastic-ramen"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "elastic-ramen")
      default:
        return "/etc/elastic-ramen"
    }
  }
  ...
  export const state = Instance.state(async () => {
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        if (!remoteConfig.$schema) remoteConfig.$schema = "https://elastic.co/config.json"
    ...
    for (const dir of unique(directories)) {
      if (dir.endsWith(".elastic-ramen") || dir === Flag.OPENCODE_CONFIG_DIR) {
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.elastic-ramen/command/", "/.elastic-ramen/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.elastic-ramen/agent/", "/.elastic-ramen/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  export const Info = z
    .object({
      ...
      command: z
        ...
        .optional()
        .describe("Command configuration, see https://elastic.co/docs/commands"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see https://elastic.co/docs/agents"),
      provider: z
    ...
    if (existsSync(legacy)) {
      await import(pathToFileURL(legacy).href, {
        ...
        .then(async (mod) => {
          ...
          result["$schema"] = "https://elastic.co/config.json"
  ...
  async function load(text: string, options: { path: string } | { dir: string; source: string }) {
    text = text.replace(/\$\{([^}]+)\}/g, (m, k) => process.env[k] ?? m)
    const original = text
    ...
    if (parsed.success) {
      if (!parsed.data.$schema && isFile) {
        parsed.data.$schema = "https://elastic.co/config.json"
