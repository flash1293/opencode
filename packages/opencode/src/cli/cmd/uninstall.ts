...
// and has been modified by Elastic NV. Changes: rebranded uninstall dialog messages from Elastic Console to RAMEN
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".elastic-ramen")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using RAMEN!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# elastic-ramen") || content.includes(".elastic-ramen/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".elastic-ramen/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".elastic-ramen/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".elastic-ramen"))
    ) {
      continue
    }
